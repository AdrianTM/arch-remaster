...
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  Arch Remaster Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
validate_version() {
    ...
    if ! [[ $clean_version =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?[a-z]*$ ]]; then
        ...
        echo "Expected formats: 1.0.0, v1.0.0, YY.MM (like 26.01), or YY.MMsuffix (like 26.01b)"
        exit 1
    fi
...
update_aur_package() {
    ...
    # Update source URL and calculate checksum
    local tarball_url="https://github.com/AdrianTM/arch-remaster/archive/refs/tags/${version}.tar.gz"
...
main() {
    ...
    if [ $# -eq 0 ]; then
        ...
        echo "Example: $0 26.02 or $0 v26.02"
        echo "         $0 26.02 --update"
        exit 1
    fi
